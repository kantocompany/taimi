...
jq -n \
  ...
  --slurpfile tools <(jq -s '[.[] | del(.verification_override, ._notes_first_pass)] | sort_by(.slug)' "${files[@]}") \
  '{
    meta: {
      version: $version,
...
for file in "${files[@]}"; do
  ...
  jq -n --argjson meta "$meta" --argjson tool "$(jq 'del(.verification_override, ._notes_first_pass)' "$file")" \
